...
- **Database:** `rusqlite` (if WASM compatible via `bundled` feature) or `sqlx` adapted for WASM environment (e.g., via `sqlite-wasm-rs`). _Note: Standard `rusqlite` might be heavy; we will investigate the best lightweight SQLite option for WASM or stick to a simpler file-based DB if SQLite overhead is too high for this phase._
